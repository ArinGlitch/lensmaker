...
export default function ItemDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <aside className="flex h-full w-full max-w-2xl flex-col border-l border-white/10 bg-[#141413]">
        {/* toolbar */}
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-white/10 px-5 py-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded border border-white/15 px-2 py-1 text-[11px] text-neutral-300 hover:bg-white/5"
          >
            Esc
          </button>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {tab === "message" ? (
            <article className="px-6 py-6">
              ...
              <div className="mt-4 flex items-start gap-3 border-b border-white/10 pb-5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/10 text-[13px] font-semibold text-neutral-200">
              ...
              <div className="mt-5 flex flex-col gap-4">
                {paragraphs.map((p, i) => (
                  <p
                    key={i}
                    className="whitespace-pre-line text-[14.5px] leading-[1.7] text-neutral-300"
                  >
                    {p}
                  </p>
                ))}
            ...
            <div className="flex flex-col gap-6 px-6 py-6">
              ...
              <section>
                ...
                <blockquote className="mt-1.5 border-l-2 border-amber-400/50 bg-[#1a1a19] px-4 py-3 font-mono text-[12.5px] leading-relaxed text-neutral-400">
                  {item.sourceExcerpt}
                </blockquote>
              </section>
              ...
              <section>
                ...
                <dl className="mt-2 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/10">
                  {fields.map((f) => (
                    <div
                      key={f.label}
                      className="flex items-center justify-between gap-4 bg-[#1a1a19] px-4 py-2.5"
                    >
